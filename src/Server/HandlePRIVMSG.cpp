...
/*   Updated: 2025/03/18 07:46:40 by bmetehri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::handlePRIVMSG(Client* client, const std::vector<std::string>& params) {
	...
	if (target[0] == '#') { // Channel message
		...
		channelPtr->broadcastMessage(":" + senderNickname + "!" + username + "@" + hostname + " PRIVMSG " + target + " :" + message + "\r\n", client);
		...
		if (recipient) {
			 sendToClient(recipient, ":" + senderNickname + "!" + username + "@" + hostname + " PRIVMSG " + target + " :" + message + "\r\n");
