...
/*   Updated: 2025/03/18 07:29:48 by bmetehri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (target[0] == '#') { // Channel MODE
		...
		for (size_t i = 0; i < modeChanges.length(); ++i) { // Regular for loop
			...
			if (modeChar == '+') {
				...
				broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " " + (adding ? "+t" : "-t") + "\r\n");
				Debug::channelEvent("Topic lock mode changed", &channel, client);
			} else if (modeChar == 'i') {
				channel.setInviteOnly(adding);
				broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " " + (adding ? "+i" : "-i") + "\r\n");
				Debug::channelEvent("Invite-only mode changed", &channel, client);
			} else if (modeChar == 'k') {
				if (adding) {
					if (paramIndex < modeParams.size()) {
						channel.setKey(modeParams[paramIndex++]);
						broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " +k " + channel.getKey() + "\r\n");
					...
					channel.clearKey();
					broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " -k\r\n");
				...
				if (paramIndex < modeParams.size()) {
					...
					if (targetClient && channel.hasUser(targetClient)) {
						if (adding) {
							...
							broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " +o " + getClientNickname(targetClient) + "\r\n");
				...
				if (adding) {
					if (paramIndex < modeParams.size()) {
						try {
							...
							if (limit > 0) {
								...
								broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " +l " + Debug::intToString(limit) + "\r\n"); // Use intToString
					...
					broadcastToChannel(target, ":" + nickname + "!" + username + "@" + hostname + target + " -l\r\n");
